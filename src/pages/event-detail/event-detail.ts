...
export class EventDetailPage {
  ...
  constructor(public platform: Platform, public navCtrl: NavController, public navParams: NavParams, public db: AngularFireDatabase, public alertCtrl: AlertController, public geolocation: Geolocation, public contaData: ContaProvider, private storage: Storage, public loadingCtrl: LoadingController) {
  ...
  conf(){
    if ( this.confirm ){
      ...
      this.userConf.push({event: this.id, check: false, criador: this.e['criador']});
      this.confirm = true;
    }
  }
        ...
        if ( (latitude*-1) <= ((lat*-1)+0.0003) && (latitude*-1) >= ((lat*-1)-0.0003) && (longitude*-1) <= ((lng*-1)+0.0003) && (longitude*-1) >= ((lng*-1)-0.0003) ){
          ...
          if ( this.e['coin'] ){
            ...
            this.userConf.forEach(us => {
              us.forEach(u => {
                console.log(u.criador)
                console.log(this.e['criador'])
                console.log(u.check)
